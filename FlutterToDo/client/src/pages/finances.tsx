  ...
  const getFilteredTransactions = () => {
    ...
    switch (timePeriod) {
      case "week":
        ...
        return transactions.filter((t: any) => new Date(t.date) >= oneWeekAgo);
      case "month":
        const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
        return transactions.filter((t: any) => new Date(t.date) >= startOfMonth);
      case "quarter":
        const startOfQuarter = new Date(now.getFullYear(), Math.floor(now.getMonth() / 3) * 3, 1);
        return transactions.filter((t: any) => new Date(t.date) >= startOfQuarter);
      case "year":
        const startOfYear = new Date(now.getFullYear(), 0, 1);
        return transactions.filter((t: any) => new Date(t.date) >= startOfYear);
      case "all":
        return transactions;
      default:
        const defaultStartOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
        return transactions.filter((t: any) => new Date(t.date) >= defaultStartOfMonth);
    }
  };
