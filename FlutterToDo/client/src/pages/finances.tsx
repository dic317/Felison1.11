...
export default function Finances() {
  ...
  const getFilteredTransactions = () => {
    ...
    const filtered = transactions.filter((t: any) => {
      ...
      const isInPeriod = isValid && transactionDate >= cutoffDate;
      ...
      console.log('🔍 Date filter check:', {
        ...
        isValid,
        isInPeriod,
        timeDiff: isValid ? (transactionDate.getTime() - cutoffDate.getTime()) / (1000 * 60 * 60 * 24) : 'invalid'
      });
  ...
  const getMonthFilteredTransactions = () => {
    return transactions.filter((t: any) => {
      ...
      if (!isValid) {
        console.log('❌ Invalid date in month filter:', t.date, t.createdAt);
        return false;
      }
      
      const matches = transactionDate.getMonth() === selectedMonth && transactionDate.getFullYear() === selectedYear;
