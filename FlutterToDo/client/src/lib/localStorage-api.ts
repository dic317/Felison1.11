...
class LocalStorageAPI {
  ...
  addTransaction(transaction: Omit<Transaction, 'id' | 'createdAt'>): Transaction {
    ...
    console.log('✅ Transaction saved to localStorage:', newTransaction.id);
  ...
  addEvent(event: Omit<CalendarEvent, 'id' | 'createdAt'>): CalendarEvent {
    ...
    const newEvent: CalendarEvent = {
      ...event,
      id: Date.now(),
      category: event.type || 'общее', // Используем type как category если нет category
      createdAt: new Date().toISOString()
    };
    
    events.push(newEvent);
    this.saveToStorage('felison_events', events);
    console.log('✅ Event saved to localStorage:', newEvent.id, newEvent);
    return newEvent;
  }
