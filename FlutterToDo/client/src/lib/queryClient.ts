...
export async function apiRequest(
  ...
  // Принудительно используем localStorage API для всех production доменов
  const isVercelApp = window.location.hostname.includes('vercel.app');
  const isNetlifyApp = window.location.hostname.includes('netlify.app');
  const isProductionDomain = isVercelApp || isNetlifyApp;
  
  // В production всегда используем localStorage
  if (isProductionDomain && url.startsWith('/api/')) {
    console.log('✅ Using localStorage API for production:', window.location.hostname);
    return simulateApiRequest(method, url, data);
  }

  // Для development пробуем сервер с fallback на localStorage
  try {
